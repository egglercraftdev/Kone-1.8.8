...
import net.kone.GuiGithubNotAvailable;
...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback {
	...
	private void addSingleplayerMultiplayerButtons(int parInt1, int parInt2) {
		...
		GuiButton singleplayerButton;
		this.buttonList.add(singleplayerButton = new GuiButton(1, this.width / 2 - 100, parInt1 + parInt2 * 0,
				I18n.format("menu.singleplayer", new Object[0])));
		...
		GuiButton btn;
		this.buttonList.add(btn = new GuiButton(14, this.width / 2 - 100, parInt1 + parInt2 * 2,
				I18n.format("menu.forkOnGitlab", new Object[0]), true));
	...
	protected void actionPerformed(GuiButton parGuiButton) {
		...
		if (parGuiButton.id == 14) {
			this.mc.displayGuiScreen(new GuiGithubNotAvailable(this, this.mc));
		}
	...
	private void drawPanorama(int parInt1, int parInt2, float parFloat1) {
		...
		GlStateManager.gluPerspective(120.0F, (float) this.mc.displayWidth / (float) this.mc.displayHeight, 0.05F, 10.0F);
		...
		//GlStateManager.tryBlendFuncSeparate(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, 0, 0);
		byte b0 = 8;
	...
	public void drawScreen(int i, int j, float f) {
		...
		if (this.isDefault || (double) this.updateCounter < 1.0E-4D) {
			...
			this.drawTexturedModalRect(k + 0, 5, 0, 70, 256, 100);
			//this.drawTexturedModalRect(k + 155, b0 + 0, 0, 45, 155, 44);
		}
		...
		String s = "kone Client 1.8.8 "+EaglercraftVersion.projectForkVersion;
		this.drawString(this.fontRendererObj, s, 7, this.height - 15, 11184810);
		...
		this.drawString(this.fontRendererObj, s1, this.width - this.fontRendererObj.getStringWidth(s1) - 7,
