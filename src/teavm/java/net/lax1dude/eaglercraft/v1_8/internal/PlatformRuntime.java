...
public class PlatformRuntime {
	...
	public static void create() {
		...
		for(int i = 0; i < epkFiles.length; ++i) {
			...
			//onDecompileEPKProgressUpdate();
			HTMLDocument docElem = Window.current().getDocument();
			HTMLElement downloadPercentElem = docElem.getElementById("downloadPercent");
			hideProgressbar();
			downloadPercentElem.setInnerHTML("Decompressing Assets... (Can take up to 2 minutes)");
	...
	@JSBody(params = { }, script = "document.getElementById('loadBarInside').style.display = 'none'; document.querySelector('#loadDiv > div:nth-child(2)').style.display = 'none'; ")
	public static native void hideProgressbar();
	...
	@JSBody(params = { }, script = "mojangScreen()")
	public static native void changeToMojangScreen();

	@JSBody(params = { }, script = "changeToCanvas()")
	public static native void changeToCanvas();
	...
	private static void downloadRemoteURI(String assetPackageURI, final AsyncCallback<ArrayBuffer> cb) {
		...
		TeaVMUtils.addEventListener(request, "progress", new EventListener<Event>() {
			...
			public void handleEvent(Event evt) {
				...
				//System.out.println(epkSize);

				//System.out.println(epkSizeMinusDivide);
				Event temporary = evt;
				
				editDownloadPercent(temporary, epkSize);
	...
	@JSBody(params = { "first", "second" }, script = "editDownloadPercent(first, second)")
	public static native void editDownloadPercent(Event first, int second);
