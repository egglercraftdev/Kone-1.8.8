...
public class PlatformInput {
	...
	public static void pressAnyKeyScreen() {
		if(mouseEvents.isEmpty() && keyEvents.isEmpty()) {
			EarlyLoadScreen.paintEnable();
			
			while(!checkKoneClientInput()) {
					//consoleLog("Sleeping");
					EagUtils.sleep(100l);
	...
	@JSBody(params = {}, script = "return keyPressedFunc()")
	private static native boolean checkKoneClientInput();

	@JSBody(params = { "string" }, script = "console.log(string)")
	private static native void consoleLog(String string);

	@JSBody(params = { "bool" }, script = "console.log(bool)")
	private static native void consoleLog(boolean bool);
